...
public class Player {
    ...
    private Move produceRandomMove(Board board){
        ...
        right = random.nextInt(board.getRowLength(row) - left +1);
        return (new Move(row,left,left+right));
    }

    /**
     * help method for taking only one stick
     * @param board The current game board
     */
    private Move takeOneStick(Board board){
        for(int i = 1; i<=board.getNumberOfRows(); i++){
            for (int j = 1; j<=board.getRowLength(i); j++){
                if(board.isStickUnmarked(i,j)){return (new Move(i,j, j));}
            }
        }
        return null;
    }
    ...
    private Move produceSmartMove(Board board){
        //check the number of remaining sticks in board
        if(board.getNumberOfUnmarkedSticks()%2 == 0){return takeOneStick(board);}
        //Making step of taking one stick
        else {
            for (int i = 1; i <= board.getNumberOfRows(); i++) {
                for (int j = 1; j < board.getRowLength(i); j++) {
                    if (board.isStickUnmarked(i, j) && board.isStickUnmarked(i, j + 1)) {
                        return (new Move(i, j, j + 1));//Making step of taking 2 sticks in a row
                    }
                }
            }
        }
        //case in which there are an odd number of sticks, but not in sequence so the player will
        //take one stick
        return takeOneStick(board);
    }
    ...
    private Move produceHumanMove(Board board){
        ...
        int userChoice, userRow = 0, userLeft = 0, userRight = 0;
        //Is used as an indicator to check whether variable absorption was completed the next step in favor
        //of the human player
        ...
        userMove = new Move(userRow,userLeft,userRight);
        return userMove;
    }
    ...
    private Move produceHeuristicMove(Board board){
        ...
        int numRows = board.getNumberOfRows();//מקבל שורות מהלוח מפסר
        int[][] bins = new int[numRows][BINARY_LENGTH];//מערך של שורות
